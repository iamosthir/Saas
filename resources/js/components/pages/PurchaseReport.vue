    ...
    <div class="col-md-12 col-lg-12">
      <div class="card">
        <div class="card-header">
          <h6 class="mb-0">تقرير المشتريات</h6>
        </div>
        <div class="card-body">
            ...
            <table class="table table-hover table-striped table-bordered">
              ...
              <tbody>
                <tr v-for="(sale, i) in rows" :key="sale.id">
                  ...
                  <td>{{ Number(sale.total_paid || 0).toLocaleString() }} IQD</td>
                  <td>
                    <span :class="{'text-danger': dueAmount(sale) > 0}">
                  ...
                  <td>
                    <a target="_blank"
                      class="btn btn-outline-secondary btn-sm" :href="`/dashboard/print-supplierinvoice/${sale.id}`">
                      <i class="fas fa-print"></i> طباعة الفاتورة
                    </a>
                    <button v-if="dueAmount(sale) > 0"
                      class="btn btn-sm btn-success ms-1" @click="openPaymentModal(sale)">
                      <i class="fas fa-plus"></i> إضافة دفعة
                    </button>
                    <button
                      class="btn btn-sm btn-info ms-1"
                      @click="openListModal(sale)">
                      <i class="fas fa-list"></i> الدفعات
                    </button>
                  </td>
                </tr>
          ...
          <form @submit.prevent="submitPayment">
            ...
            <div class="modal-body">
              <div v-if="paymentTargetInvoice">
                ...
                <div><b>المورّد:</b> {{ paymentTargetInvoice.supplier_name }}</div>
              ...
              <div class="form-group mt-3">
                ...
                <input type="number" class="form-control" v-model="paymentForm.amount" required min="1" :max="dueAmount(paymentTargetInvoice)">
              </div>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div v-if="listTargetInvoice">
              ...
              <div><b>المورّد:</b> {{ listTargetInvoice.supplier_name }}</div>
            ...
            <table v-if="listTargetInvoice" class="table table-bordered table-sm mt-3">
              <thead>
                <tr>
                  <th>#</th>
                  <th>المبلغ</th>
...
export default {
  ...
  methods: {
    async getReport(page = 1) {
      ...
      await axios.get('/dashboard/api/purchase', { params }).then(resp => {
        const data = resp.data;
        this.paginateData = data;
        this.rows = data.data || [];
        console.log(this.rows);
        
        this.totalThisMonth = data.total_purchases_month || 0;
      }).catch(() => {
        this.paginateData = {};
        this.rows = [];
    ...
    async submitPayment() {
      ...
      await axios.post('/dashboard/api/purchase-payments', payload)
        .then(resp => resp.data)
        .then(data => {
          if (data.status === "ok") {
  ...
  filters: {
    formatDate(val) {
      if (!val) return '';
      // Format: yyyy-mm-dd HH:MM or as you like
